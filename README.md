...
В каждой папке лежит `SKILL.md`, инструкция для AI-агента. После установки агент получает чёткий порядок работы, входные данные, формат результата и ограничения. Набор не подключается к вашим аккаунтам, не публикует контент и не читает данные сам. Вы приносите тексты, комментарии, тему или готовый сценарий. Навык возвращает отдельный рабочий артефакт.

Пять артефактов проходят по цепочке: `VOICE-PROFILE`, `AUDIENCE-INTEL`, `HOOK-BRIEF`, `SCRIPT-PACK` и набор версий для площадок. Их можно сохранить, проверить и передать следующему навыку без повторного объяснения задачи.

## с какими проблемами работает набор

- голос распадается, когда посты, письма и ролики пишутся в разных чатах;
- темы выбираются по памяти, хотя в комментариях уже есть вопросы и живые формулировки;
- первая строка обещает одно, а сценарий раскрывает другое;
- видео превращается в карусель заменой переносов строк;
- каждый новый запрос снова требует объяснять агенту контекст, тон и аудиторию.

Связка сохраняет происхождение материала. Фразы аудитории идут в хук. Хук и факты идут в сценарий. Сценарий даёт основу для других площадок. Голос автора остаётся на каждом шаге.

## большой гид по всем пяти навыкам

Ниже полный разбор пакета: входы, результаты, порядок работы, сложные случаи, готовые запросы, передача результата дальше и контроль перед публикацией.
...
После установки навыки не подключаются к Telegram, Instagram, рассылке или базе данных. Они не публикуют материал и не получают доступ к аккаунтам. Агент начинает применять инструкции, когда в чате появляется подходящая задача и входные материалы. Автор приносит тексты, комментарии, тему или готовый сценарий. Навык разбирает их и возвращает рабочий артефакт.

Пакет покрывает путь от сырого контента до набора публикаций:
...
комментарии / DM / отзывы ─> AUDIENCE-INTEL ─> HOOK-BRIEF ─> SCRIPT-PACK ─> форматы
                                                │
                                                └──── голос остаётся в каждом шаге
```

Можно поставить один навык. Например, `repurpose` пригодится, когда уже есть готовый ролик и нужно подготовить письмо или карусель. Полная связка полезна там, где контент выходит регулярно и одна мысль проходит несколько форматов.

### откуда берётся проблема

Обычно материалы лежат в разных местах. В заметках остаются темы. В комментариях копятся вопросы. В голосовых есть нормальные объяснения. Тексты для постов, видео и писем собираются отдельными запросами, поэтому у них расходится голос, а исходная мысль теряется по дороге.

Первый запрос часто даёт общий текст. Второй приносит длинный список хуков без связи с аудиторией. Третий превращает видео в карусель заменой переносов строк. В результате публикации выглядят как разные заготовки, хотя должны поддерживать одну линию.

Связка решает эту организационную проблему через промежуточные артефакты. Каждый этап оставляет результат, который можно проверить, поправить и отдать дальше. Не приходится заново пересказывать агенту, кто читатель, как звучит автор и что уже было решено.

![одна мысль проходит пять этапов](assets/svg/content-machine-handoff.svg)

### с чего начать за один рабочий вечер
...
Сначала запустите `voice-print` на собственных материалах. Потом передайте реакцию аудитории в `audience-dig`. Эти два результата дают основу для темы и первого хода. После этого выберите одну тему, соберите хук, разверните его в сценарий и подготовьте версии для площадок.

Первые проходы не требуют идеального архива. Важно сохранить получившиеся `VOICE-PROFILE`, `AUDIENCE-INTEL`, `HOOK-BRIEF` и `SCRIPT-PACK` отдельными файлами или заметками. Они позволяют обновлять один этап без повторной работы по всему материалу.

### глава 1. voice-print фиксирует узнаваемый голос

##### когда он нужен

`voice-print` ставят перед созданием серии постов, видео, писем или сценариев. Он полезен, когда несколько агентов или людей готовят контент, а тексты начинают звучать по-разному. Навык строит наблюдаемый профиль письма. В нём остаются слова, ритм, обращение к читателю, типичные конструкции и границы тона.
...
Вместе с текстами можно указать задачу. Например: «нужен профиль для будущих Reels и постов в Telegram» или «сравни ранние заметки с текстами за последний месяц». Если прислано меньше трёх образцов, результат остаётся предварительным. Навык должен назвать низкую уверенность, а не делать вид, что двух абзацев достаточно.
...
Результатом становится `VOICE-PROFILE`. Его можно хранить рядом с контент-планом и прикладывать к следующим задачам. Профиль не должен превращаться в длинный литературный разбор. Он нужен как набор правил, по которым другой агент сможет собрать новый материал.
...
##### как навык собирает профиль

Сначала он читает все образцы целиком. Выдёргивание пары любимых фраз даёт искажённую картину. Потом отмечает повторяющиеся привычки по шести слоям.
...
Дальше идёт отношение к читателю. Автор пишет «ты», «вы», без обращения или через личный опыт. Затем фиксируется структура. Некоторые начинают с эпизода, другие с тезиса, третьи сначала показывают ошибку. Последний слой содержит запреты. Он защищает последующие тексты от приёмов, которых в образцах нет.
...
В `hook-lab` уходят лексика, ритм, обращение к читателю и запреты. В `script-smith` добавляется способ строить начало и объяснять сложную мысль. В `repurpose` профиль идёт вместе с готовым исходником, чтобы смена формата не сделала текст чужим.
...
### глава 2. audience-dig превращает входящие в карту тем
...
`audience-dig` принимает массив реплик и собирает `AUDIENCE-INTEL`. Это отчёт о повторениях, эмоциональных маркерах, явных вопросах, конкретной похвале и формулировках проблемы на языке аудитории. В нём остаётся материал, из которого можно выбрать тему и первый хук.
...
Навык читает весь массив и отделяет содержательные сообщения от шума. Спам, одинаковые реакции и реплики без контекста не должны влиять на вывод. Затем похожие сообщения группируются, но вопрос не смешивается с болью только потому, что оба содержат одно слово.
...
Дальше навык собирает прямые вопросы, проблемные формулировки и конкретные причины похвалы. В конце появляются темы и несколько хуков, которые опираются на реальные слова аудитории. Имена и другие идентифицирующие детали в финальный отчёт не попадают.
...
`hook-lab` получает одну или две реальные фразы, выбранную боль и площадку. `script-smith` получает более широкий контекст: почему тема важна, какие вопросы нужно закрыть и какие факты потребуются. `repurpose` работает уже с готовым материалом, где исходная проблема не потеряна.
...
### глава 3. hook-lab собирает первый ход под конкретный материал

##### почему отдельный навык для первой строки
...
`hook-lab` берёт тему, площадку, аудиторию и голос. На выходе появляется не россыпь случайных заголовков, а подготовленный `HOOK-BRIEF`. В нём видно, кому адресована первая строка, какую мысль нужно раскрыть дальше и где у выбранной формулы есть риск.
...
Если площадка не указана, навык может принять короткое видео как рабочее допущение и назвать его. Для треда, карусели или YouTube контекст нужно задать отдельно. Первый кадр и первая строка текста работают по-разному.

##### что приходит в ответ

Финальный артефакт можно оформить так:
...
Потом навык вытаскивает боль, желание, конфликт или подтверждённый факт. Он сверяет их с формулами и убирает натянутые варианты. Дальше проверяет обещания, цифры и payoff. Вариант готов только когда понятно, чем тело материала отвечает на первую строку.
...
**автор бросает контент через неделю.** Сырой запрос просит «мотивационный ролик». В комментариях повторяется «я уже пробовал, но опять бросил». `hook-lab` строит первый ход вокруг причины: план требует ежедневного придумывания темы. Тело объясняет очередь из готовых тем.

**B2B-команда не показывает кейсы.** Сырой запрос говорит «нужен ролик про кейсы». Вход уточняется до проблемы: команда хранит результат в чатах и не собирает доказательства. Первый ход ведёт к схеме сохранения факта после каждого проекта.

##### где навык останавливается
...
### глава 4. script-smith раскладывает мысль по времени
...
Сначала навык берёт выбранный хук. Потом считает, сколько мыслей помещается в выбранное время. Каждой мысли нужен факт, пример, экран или демонстрация. Если доказательства нет, формулировка становится точнее либо тезис уходит в отдельный ролик.
...
### глава 5. repurpose собирает новую форму для той же мысли
...
##### как навык работает с разными исходниками
...
Короткая мысль не растягивается до письма, если ей не хватает материала. Узкая техническая тема может потерять важные детали при сжатии, тогда навык предупреждает об этом. Один исходник не обязан стать всеми форматами сразу. Форматы выбираются по задаче, а не ради количества.
...
`hook-lab` превращает эту фразу в первый ход о контенте, который ломается ещё на этапе планирования. `script-smith` собирает ролик на сорок пять секунд с демонстрацией очереди тем. `repurpose` делает из сценария карусель с шагами, Telegram-пост с личным контекстом и письмо с одним упражнением.
...
4. сохраните четыре промежуточных артефакта после каждого шага;
...
- [установка и совместимость](installation.md)
- [рабочие сценарии](workflows.md)
- [контракты всех навыков](skill-reference.md)
- [примеры входов и результатов](examples/)
- [приватность и этика](privacy-and-ethics.md)
...
Для полного понимания набора откройте [подробный гид](docs/content-machine-guide.md). Он показывает один проход от комментария до серии публикаций и объясняет, когда достаточно одного навыка.
...
### 4. `script-smith` считает время и смысловые блоки
